...
def run_pipeline(script_path):
    ...
    try:
        ...
        # BUT, we can use `subprocess` to run `python -m scriptpulse.pipeline.runner <file>` and capture stdout/json.
...
if __name__ == "__main__":
    ...
    for name, path in scripts.items():
        ...
        cmd = [sys.executable, "-m", "scriptpulse.pipeline.runner", path]
        try:
